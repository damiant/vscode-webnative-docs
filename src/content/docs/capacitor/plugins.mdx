...
<Video src="https://videos.webnative.dev/plugins.mp4" alt="Plugins" controls />
...
You may need to install a dependency that is not a plugin. If your search is an exact match for an npm package, then it will also show (e.g., try `angularx-qrcode`). You can then click `Install` to add that dependency to your project.
...
4. Is the version considered stable? (i.e., version 0.x)
...
-- If you are using the latest version of a plugin you may see `Up To Date`. Plugins are checked daily, so it's possible that a plugin released today may still show `Up To Date`.
...
1. Be sure to specify an appropriate license type in the `license` property (e.g., `MIT`, `Apache-2.0`, `BSD`).
1. Set the GitHub repo URL in the `repository` > `url` property.
1. Set the URL to where to file issues in the `bugs` > `url` property.
1. Be sure to set the author's name in the `author` property (e.g., `author: { name: 'John Smith' }`).
...
-- Avoid keyword stuffing (e.g., using keywords like `plugin` or terms unrelated to your plugin). These are automatically excluded during indexing.
